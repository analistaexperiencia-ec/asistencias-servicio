  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <aside 
        ...
        } bg-white shadow-2xl flex flex-col transition-all duration-300 z-50 border-r border-slate-200`}
      >
        ...
        <div className="p-5 space-y-4 bg-slate-50 border-b border-slate-200">
          <div className="relative">
            <input 
              ...
              className="w-full pl-10 pr-12 py-3 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#1A365D] focus:border-transparent outline-none transition-all"
          ...
          {aiMessage && (
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800 animate-fade-in flex items-start gap-2">
              <span className="text-lg">💡</span>
              <p>{aiMessage}</p>
              <button onClick={() => setAiMessage(null)} className="ml-auto text-blue-400 hover:text-blue-600">&times;</button>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1">
          <div className="flex items-center justify-between mb-4 px-1">
            ...
            {userLocation && <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 uppercase">Cerca de ti</span>}
          </div>
          
          {filteredProviders.length === 0 ? (
            <div className="text-center py-20">
              <div className="text-4xl mb-4">🔍</div>
              <p className="text-slate-500 font-medium">No hay proveedores con estos filtros</p>
              <button 
                onClick={() => setFilters({ search: '', categoria: '', provincia: '', ciudad: '' })}
                className="mt-4 text-[#1A365D] text-sm font-bold underline"
              >
                Limpiar filtros
              </button>
            </div>
      ...
      <main className="flex-1 relative">
        <button 
          ...
          className={`absolute top-4 left-4 z-20 bg-white p-3 rounded-full shadow-xl text-[#1A365D] border border-slate-200 transition-all ${isSidebarOpen ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`}
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" /></svg>
        </button>

        <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
           <div className="bg-white px-4 py-2 rounded-xl shadow-lg border border-slate-200 flex items-center gap-3">
              <div className="w-3 h-3 bg-[#1A365D] rounded-full animate-pulse"></div>
              <span className="text-sm font-bold text-slate-700">Sistema Activo - Ecuador</span>
           </div>
        </div>
